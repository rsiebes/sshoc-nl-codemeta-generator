"""Module 51: version - Extract software version"""
import logging
...
class VersionExtractor:
    ...
    def extract(self) -> Optional[str]:
        ...
        releases_url = self.repo_data.get("releases_url", "")
        if releases_url:
...
def extract(repo_data: Dict[str, Any], repo_files: Dict[str, str] = None, **kwargs) -> Optional[str]:
    try:
        return VersionExtractor(repo_data).extract()
...
def get(repository_url: str) -> Dict[str, Optional[str]]:
    try:
        ...
        return {"version": result} if result else {}
    except Exception as e:
        logger.error(f"Error: {str(e)}")
        return {}
