"""
Reference Publication Property Module

Handles extraction and validation of the 'reference_publication' Codemeta property.
...
class ReferencePublicationMetadata(BaseMetadata):
    """Handles reference_publication metadata extraction and validation."""

    CODEMETA_PROPERTY = 'rPublication'
    CODEMETA_TYPE = 'schema:ItemList'
    REQUIRED = False

    def extract(self) -> Dict[str, Any]:
        """
        Extract reference_publication from raw data.

        Returns:
            Dictionary with 'rPublication' key containing the extracted value
        """
        # Try to extract from raw data
        value = self._get_value('rPublication')
    ...
    def _validate_metadata(self) -> None:
        """Validate reference_publication metadata."""
